...
solutionpopup::solutionpopup(QWidget *parent)
  ...
  ui->pointsView->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  ...
  connect(scene, &GraphicScene::pointsUpdated, scene,
          &GraphicScene::updatePointsModel);

  connect(ui->showCoordinatesRadio, SIGNAL(toggled(bool)), this,
          SLOT(onShowCoordinatesRadioToggle(bool)));
  connect(ui->scaleInput, SIGNAL(editingFinished()), this,
          SLOT(onScaleInputEditFinished()));
  connect(scene, SIGNAL(zoomChanged(double)), this,
          SLOT(onZoomChanged(double)));
  connect(ui->solveButton, SIGNAL(clicked()), this,
          SLOT(OnSolveButtonClicked()));
}
...
void solutionpopup::OnSolveButtonClicked() {
  ...
  if (result == 1)
    QMessageBox::warning(this, "warning",
                         "Недостаточно точек для построения треугольника");
  else if (result == 2)
    QMessageBox::warning(
        this, "warning",
        "Все полученные треугольники вырожденные, невозможно решить задачу");
  else
    this->scene->update();
}
